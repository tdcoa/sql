									...
									FROM dbc.dbcinfo info,
										(
											...
											FROM pdcrinfo.ResUsageSpma
												WHERE ( ( THEDATE > ('2020-09-13' ) AND THETIME >= 0 ) OR	( THEDATE = ('2020-09-13' ) ) )
												AND   ( ( THEDATE = ('2020-09-28') AND THETIME <= 240000 ) OR ( THEDATE < ('2020-09-28') ) )
												GROUP BY 1,2,3,4,5
										) spma_dt
...
CB_DAILY_TCORE_METRICS c /* S c                                                             Only the portion from DBQL Log */
				inner join
					(
						...
						where logdate between '2020-09-13' and '2020-09-28'
						group by 1
					)b
...
(
	SELECT
		...
		WHERE ql.LogDate between '2020-09-13' and '2020-09-28'
...
SELECT
 ...
 /* , count(*)  CountOfUses */
			...
			AND dbql.LogDate between '2020-09-13' and '2020-09-28'
			GROUP BY 1,2,3,4,5
			...
			** for short queries either */
			...
			(
						...
						JOIN "consumption_stepgrp.csv" sg
							...
							AND SQ.LogDate between '2020-09-13' and '2020-09-28'
...
SELECT        DISTINCT
   ...
   left outer join CB_ComplexityScore c
	...
	on 1= 1
	where l.logdate between '2020-09-13' and '2020-09-28'
...
where START_DT >=
(case when 1 between 5 and 6 then (select  ADD_MONTHS(QuarterBegin, (SELECT cast(COLUMNVALUE as int) FROM  CB_DIM_CONSTANT_REF WHERE COLUMNNAME='QTR_OFFSET')) from CB_DIM_BusinessCalendar where calendar_date = '2020-09-13')
	  else '2020-09-13' end)
and END_DT <=
...
from CB_DIM_BUDGET_REF
where START_DT >=
(case when 2 between 5 and 6 then (select  ADD_MONTHS(QuarterBegin, (SELECT cast(COLUMNVALUE as int) FROM  CB_DIM_CONSTANT_REF WHERE COLUMNNAME='QTR_OFFSET')) from CB_DIM_BusinessCalendar where calendar_date = '2020-09-13')
	  else '2020-09-13' end)
and END_DT <=
