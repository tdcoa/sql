LOCK	ROW FOR ACCESS 
SEL	logdate AS logdate,  AVG(AvgCPUPct) AS cpu_peak_value 
FROM	(
SELECT	'SiteId' AS SiteID, CURRENT_DATE(FORMAT'YYYY-MM-DD')(CHAR(10)) AS ReportDate,
		...
		4)) AS AvgCPUPct, MovingAvg , Trend(DECIMAL(18,4)) AS Trend,
		ReserveX, 
CASE	
...
FROM	  pdcrinfo.ResUsageSPMA_hst s1, sys_calendar.CALENDAR c1 
