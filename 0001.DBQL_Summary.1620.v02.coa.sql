...
(
  ...
  left join "dim_app.coa.csv" as p
    on (case
        ...
        when p.Pattern_Type = 'RegEx' and regexp_substr(o.AppID, p.Pattern,1,1,'i')>0 then 1
        else 0 end) = 1
...
(
  ...
  left join "dim_statement.coa.csv"  as p
    on (case
        ...
        when p.Pattern_Type = 'RegEx' and regexp_substr(o.StatementType, p.Pattern,1,1,'i')>0 then 1
        else 0 end) = 1
...
(
  ...
  left join "dim_user.coa.csv" as p
    on (case
        ...
        when p.Pattern_Type = 'RegEx' and regexp_substr(o.UserName, p.Pattern,1,1,'i')>0 then 1
        else 0 end) = 1
